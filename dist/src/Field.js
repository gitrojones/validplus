...
class VPField extends Validatable_1.Validatable {
    ...
    parseInput() {
        ...
        const inputRules = filterNullObject_1.filterNullObject({
            ...
            required: toBoolean_1.toBoolean(this.$input.getAttribute('required'), null)
        });
    ...
    setInput() {
        debug_1.debug('[VPField] Querying inputs');
        ...
        if (input.length > 0)
            debug_1.debug('[VPField] Found input', input);
        if (select.length > 0)
            debug_1.debug('[VPField] Found select', select);
        if (textarea.length > 0)
            debug_1.debug('[VPField] Found textarea', textarea);
        ...
        let _input = (input.item(0) || select.item(0) || textarea.item(0));
