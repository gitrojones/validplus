...
const Validator = function(options, form = null) {
  if (form === null) {
    debug(
      '[Validator] Non-strict fieldset matching.',
  ...
  if (Array.isArray(options.validationInputs)) {
    this._options.validationInputs = this._options.validationInputs.concat(
      options.valiationInputs
    );
  }
  ...
  if (this._isValid) {
    ...
    if (this.options.scrollTo === true) {
      ...
      if (this.options.scrollAnchor instanceof Element) {
        ...
        const firstElement = this._fieldsets.filter(
          f => f._isValid === false
        )[0];
...
// TODO: Add MutationObserver on children
Validator.prototype.createFieldset = function(
  fs,
  strategy,
  options,
  fields,
...
const ElementOrID = function(ElorID, form = null) {
  ...
  if (typeof ElorID === 'string') {
    let f =
      form !== null && form instanceof Element
